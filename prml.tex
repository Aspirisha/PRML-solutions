...
Suppose all the variables are fixed except $x_j$. Let us denote $\boldsymbol{x_0}$ the set of $\boldsymbol{x}$ variables with $x_j = 0$, and $\boldsymbol{x_1}$ the same set, but with $x_j = 1$. Then 

\begin{align}
	E(\boldsymbol{x_1},\boldsymbol{y}) - E(\boldsymbol{x_0},\boldsymbol{y}) = h (1 - 0) - \beta \sum\limits_{i : ne(j)) } 1\cdot x_i - \eta 1 \cdot y_j = h - \beta \sum\limits_{i : ne(j)) } x_i - \eta y_j
\end{align}
